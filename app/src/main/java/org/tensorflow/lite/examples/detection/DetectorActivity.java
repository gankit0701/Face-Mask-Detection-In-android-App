...
//        final int inHeight = transpose ? srcWidth : srcHeight;
    ...
    Log.d("Matrix Value", String.valueOf(matrix));
    return matrix;
  ...
  private void onFacesDetected(long currTimestamp, List<Face> faces) {
    ...
    for (Face face : faces) {
      ...
      if (boundingBox != null && goodConfidence) {
        ...
        Log.d("Matrix Potrait", String.valueOf(matrix));
        ...
        if (resultsAux.size() > 0) {
          ...
          if (conf >= 0.6f) {
            ...
            if (result.getId().equals("0")) {
              ...
              txtMessage.setText("Happy Independence Day \n Keep India Safe");
        ...
        if (getCameraFacing() == CameraCharacteristics.LENS_FACING_FRONT) {
          ...
          Log.d("BoundingBox", String.valueOf(boundingBox));
          flip.mapRect(boundingBox);

        }

        final Classifier.Recognition result = new Classifier.Recognition(
                "0", label, confidence, boundingBox,imaage);

        result.setColor(color);
        result.setLocation(boundingBox);
        result.setImage(imaage);
